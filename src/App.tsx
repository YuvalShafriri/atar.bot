...
const AiSpot: React.FC<{ spotId: 'tips' | 'ideas'; onQuery: (q: string) => Promise<string>; exampleQueries?: string[] }> = ({ spotId, onQuery, exampleQueries }) => {
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');
  const [loading, setLoading] = useState(false);

  const config = {
    tips: { title: 'Ask the bot for tips', description: 'Request short, actionable tips.', placeholder: 'e.g. Give me a prompt template for step 1' },
    ideas: { title: 'Brainstorm ideas', description: 'Generate new ideas based on examples.', placeholder: 'e.g. Suggest an interactive timeline feature' }
  }[spotId];
  ...
  const handleAsk = async (q?: string) => {
    ...
    try { const r = await onQuery(query); setOutput(r); } catch (e) { setOutput('Error calling model.'); }
    setLoading(false);
  };
  ...
  return (
    <div className="ai-spot mt-2">
      ...
      <div className="flex gap-2">
        <input value={input} onChange={e => setInput(e.target.value)} placeholder={config.placeholder} className="flex-grow p-2 border rounded" />
        <button onClick={() => handleAsk()} disabled={loading} className="btn">Ask</button>
      </div>
      {exampleQueries && exampleQueries.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-2">
          {exampleQueries.map((q, i) => (
            <button key={i} className="px-2 py-1 rounded border text-xs bg-gray-100" onClick={() => handleAsk(q)}>{q}</button>
          ))}
        </div>
      )}
      <div className="p-3 mt-2 bg-white rounded border min-h-[80px] whitespace-pre-wrap">{output}</div>
    </div>
  );
};
            ...
            <div className="mt-7">
                <div className="max-w-4xl mx-auto">
                    <div className="bg-white p-4 rounded-lg shadow mb-4">
                        <h2 className="text-xl font-semibold text-gray-800 m-0">Useful Workshop Links</h2>
                    </div>
                    <div className="flex flex-col gap-6">
                        <div className="bg-white p-6 rounded-lg shadow">
                            <ul className="custom-list space-y-2 text-lg mb-2">
                                <li className="icon-link"><a href="https://chatgpt.com/g/g-687366896c1c81918ebf923352f45b31-tr-bvt-yqvmvs" target="_blank" rel="noopener" className="text-blue-600 hover:underline">atar.bot &nbsp;&gt; <em>Cultural-Insites</em></a></li>
                                <li className="icon-bot relative flex items-center">
                                   ...
                                   >
                                     ...
                                     {showBrainTooltip && (
                                       ...
                                       >
                                         This file contains all process steps, guidelines, and CBSA knowledge required for execution.
                                         You can save it and upload to any chatbot environment (Gemini, Claude, ChatGPT etc.) to run the process there - recommended to use in a project environment that allows memory between project conversations.
                                       </div>
                                     )}
                                   </div>
                                </li>
                                <li className="icon-drive"><a href="https://drive.google.com/drive/folders/1cXDqYxmtaliDE1Cq9heT2u_0BRXz5Gj6?usp=sharing" target="_blank" rel="noopener" className="text-blue-600 hover:underline">Shared Workshop Folder (Google Drive)</a></li>
                                <li className="icon-graph"><a href="https://poloclub.github.io/transformer-explainer/" target="_blank" rel="noopener" className="text-blue-600 hover:underline">Transformer Mechanism Visualization for Next Word Prediction</a></li>
                        ...
                        <div className="bg-white p-0 rounded-lg shadow">
                            <div className="full-viewport-image" style={{ ['--nav-footer-height' as any]: '96px' }}>
                                <img
...
const HandsOn2Page: React.FC = () => {
  ...
  const [modalFile, setModalFile] = React.useState<{id:string,label:string,href:string} | null>(null);
    ...
    <div className="space-y-4 py-6">
      ...
      <div className="bg-white p-6 rounded shadow">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          {files.map(f => (
            <button key={f.id} onClick={() => openModal(f)} className={`w-full text-left p-4 border rounded hover:shadow ${modalFile?.id===f.id? 'border-blue-500 bg-blue-50': 'border-gray-200'}`}>
        ...
        <p className="text-sm text-gray-600">Click any card to open the page inline in a modal. Use the close button or Esc to dismiss.</p>
        ...
        {modalOpen && modalFile && (
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            >
              ...
              <div className="flex items-center justify-between px-2 border-b" style={{ height: 40, minHeight: 40 }}>
                <div className="font-medium text-sm truncate px-1">{modalFile.label}</div>
                <div className="flex items-center gap-2 pr-1">
                  <a href={modalFile.href} target="_blank" rel="noopener" className="text-sm text-blue-600 hover:underline" style={{ padding: '4px 6px' }}>Open</a>
                  <button onClick={() => setModalOpen(false)} className="p-1 rounded border" aria-label="Close modal">✕</button>
                </div>
              </div>
...
const TipsPage: React.FC = () => {
  ...
  return (
    <div id="tips" className="py-6">
      ...
      <AiSpot spotId="tips" onQuery={(q) => askTipsLLM(q, tipsList)} exampleQueries={[ 'Give me a prompt template for step 1' ]} />
    </div>
  );
};
...
const IdeasPage: React.FC = () => {
  ...
  return (
    <div id="ideas" className="py-6">
      ...
      <AiSpot spotId="ideas" onQuery={(q) => askBrainstormLLM(q, ideasList)} exampleQueries={[ 'Suggest an interactive timeline feature' ]} />
    </div>
  );
};

const DashboardPage: React.FC<any> = ({ allGraphData = {}, allGrapheCleanData = { nodes: [], edges: [] }, thematicGraphData = { nodes: [], edges: [] }, nodeColors = {} }) => {
  return (
    <div id="dashboard" className="py-6">
      <h2 className="text-2xl font-bold mb-2">Knowledge Graphs</h2>
      <p className="text-gray-600 mb-4">Graphs below show knowledge networks built from workshop assessments.</p>
      <ImprovedGraphDashboard allGraphData={allGraphData} allGrapheCleanData={allGrapheCleanData} thematicGraphData={thematicGraphData} nodeColors={nodeColors} />
    </div>
  );
};
...
const App: React.FC = () => {
  ...
  const [data, setData] = useState({ allGraphData: {}, allGrapheCleanData: { nodes: [], edges: [] }, thematicGraphData: { nodes: [], edges: [] }, nodeColors: {} });
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (!graphRes.ok || !thematicRes.ok || !allGrapheCleanRes.ok) throw new Error('Network error');
        const graphJson = await graphRes.json();
        const thematicJson = await thematicRes.json();
        const allGrapheCleanJson = await allGrapheCleanRes.json();
        setData({ nodeColors: graphJson.NODE_COLORS ?? {}, allGraphData: graphJson.allGraphData ?? {}, allGrapheCleanData: allGrapheCleanJson, thematicGraphData: thematicJson });
      } catch (e) { console.error(e); setError('Failed to load graph data.'); } finally { setLoading(false); }
    }
    loadData();
  }, []);
  ...
  useEffect(() => {
    if (page !== displayedPage) {
      setFade(false);
      const t = setTimeout(() => { setDisplayedPage(page); setFade(true); }, 200);
  ...
  const renderPage = () => {
    ...
    if (error) return <div className="text-center p-10 text-red-500 bg-red-100 rounded-md">{error}</div>;
    switch (displayedPage) {
      case 'home-en': return <HomePageEn />;
      case 'hands-on-1': return <ExperiencePageEn />;
      case 'hands-on-2': return <HandsOn2Page />;
      case 'discussion': return <DiscussionPage />;
      case 'tips': return <TipsPage />;
      case 'ideas': return <IdeasPage />;
      case 'kg-map': return <KGMapPage />;
      case 'dashboard': return <DashboardPage {...pageProps} />;
      case 'workshop-report': return <WorkshopReport />;
      default: return <HomePageEn />;
    }
  };
    ...
    <ErrorBoundary>
      <div dir="ltr">
        ...
        <footer className="site-footer text-center py-6">
          <p>© Cultural Insights Workshop, Cipa 2025 Dr. Yael Alef and Yuval Shafriri.</p>
        </footer>
      </div>
    </ErrorBoundary>
  );
