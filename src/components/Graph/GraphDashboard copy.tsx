...
import { chatGraph, GraphData, LLMMessage } from '../../services/graphQueryServiceOLD';
...
const AiSpot: React.FC<AiSpotProps> = ({ spotId, onQuery, placeholder, exampleQueries }) => {
  ...
  const config = {
    dashboard: {
      title: '',
      description: '',
      placeholder: placeholder || 'שאל את הבוט על הנכס – קבל הסבר על קשרים וערכים…'
    }
  }[spotId] ?? { title: '', description: '', placeholder: '' };
  ...
  const handleAsk = async (customInput?: string) => {
    ...
    try {
      ...
      const words = answer.trim().split(' ').filter(Boolean);
      let curr = '';
      for (let i = 0; i < words.length; i++) {
        curr += words[i] + (i < words.length - 1 ? ' ' : '');
        setOutput(curr);
        await new Promise((r) => setTimeout(r, 30));
      }
    } catch {
      setOutput('שגיאה בקבלת תשובה.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="ai-spot mt-1">
      <div className="flex gap-2">
        <input
          className="flex-grow p-2 border rounded bg-white text-sm"
          value={input}
      ...
      {output && <div className="mt-2 p-2 border rounded text-sm bg-white whitespace-pre-line">{output}</div>}
    </div>
  );
};
...
`;
  ...
  // Graph visualization effects - based on GraphDashboardO.tsx
  useEffect(() => {
  ...
  // (המשך קוד – בניית vis‑network וכו')
  // לשם קיצור, משאיר רק תצוגה פשוטה; הוסף לוגיקה הגרפית שלך כאן
  ...
  return (
    <div className="bg-white p-3 rounded-lg shadow">
      ...
      <AiSpot
        spotId="dashboard"
        onQuery={handleQuery}
        key={assetId} // Reset component when assetId changes
