...
async function askTipsLLM(question: string, tipsList: Array<{ title: string; text: string }>): Promise<string> {
    ...
    // Add instruction for a short answer and to avoid repeating existing tips
...
async function askBrainstormLLM(question: string, ideasList: any[]): Promise<string> {
    ...
    // Add instruction for a short answer and to avoid repeating existing ideas
...
// Simple Dashboard Page - only contains textual content
    ...
    return (
        <div id="dashboard" className="page active">
            ...
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-200 mb-4">
                <h3 className="text-lg font-semibold text-blue-800 mb-2">על הגרפים</h3>
                <p className="text-blue-700">
                    הגרפים שלהלן מציגים את רשתות הידע שיצרו המשתתפים בסדנאות אתר.בוט. 
                    כל גרף מייצג נכס מורשת ספציפי ומכיל צמתים המתארים ערכים, אירועים, דמויות והקשרים רלוונטיים.
                </p>
            </div>
...
הוא נבנה עבור משתתפי הסדנה, ופתוח לעיון לכל המתעניין בצומת שבין הערכת מורשת תרבותית ובינה מלאכותית.            </p>
        </div>

        <div className="mt-7">
            {/* <h2 className="text-3xl font-bold mb-6 text-gray-900 text-center">מה עשינו: סיכום המחזורים</h2> */}
            <div className="flex flex-col gap-6 max-w-4xl mx-auto">
                ...
                <div className="bg-white p-3 rounded-lg shadow">
                    {/* <h3 className="font-bold text-xl mb-2">תמצית המבואות</h3> */}
                    {/* <p>סקירה תמציתית של הרקע והמתודולוגיה שהוצגו בתחילת הסדנאות.</p> */}
                    <p className="mt-2 text-gray-700 leading-relaxed">בסדנת אתר.בוט בחנו מחדש את הדרכים שבהן אנו מעריכים את משמעותם של נכסי מורשת, תוך שימוש בשיטת CBSA (הערכת משמעות מבוססת הקשר) שמבוססת על הקשר, מידע וקריאה פרשנית. לצד זאת, ניסינו לשלב בינה מלאכותית יוצרת כשותפה קוגניטיבית – שיכולה להרחיב את המבט, לחדד ניסוחים ולחשוף הקשרים, כל עוד נשמרת ביקורת אנושית. ההתנסות בכלי אתר.בוט, שמחבר בין CBSA ל‑Gen-AI, אפשרה לנו להציץ אל עתיד שבו בינה מלאכותית משתלבת בתיעוד ושימור – מבלי לוותר על האחריות והפרשנות האנושית שבבסיסם.                   
                    </p>
     ...
     {/* <div className="flex justify-end pr-4 mt-4">
            <button onClick={() => onNavClick('dashboard')} className="mt-8 px-8 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition">צפו בתוצרים</button>
        </div> */}
    </div>
...
const IdeasPage = () => {
    const ideasList = [
        ...
        // { title: "מפה אינטראקטיבית", text: "הטמעת מפה המציגה את כל הנכסים לפי מיקומם הגיאוגרפי. ניתן יהיה לסנן את המפה לפי סוג נכס, תקופה או ערכים." },
        // { title: "בוט לניתוח תמונות", text: " כך שיוכל לקבל תמונה של המבנה ולנתח את מאפייניו האדריכליים, מצבו הפיזי או לזהות אלמנטים ייחודיים." },
        ...
        // { title: "שילוב עם מאגרי מידע", text: "חיבור האתר למאגרי מידע חיצוניים (כמו ארכיונים דיגיטליים, ספריות או מאגרי תמונות היסטוריות) כדי להעשיר את המידע על כל נכס." },
